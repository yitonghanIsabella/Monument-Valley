...
void TriMesh::computeVertexNormals()
{
	...
	{
		vertex_normals[i] = glm::normalize(vertex_normals[i] / length(vertex_normals[i]));
	}
	// 球心在原点的球法向量为坐标
...
void TriMesh::generateCube()
{
	...
	texture_index.push_back(vec3i(0, 3, 1));
	texture_index.push_back(vec3i(0, 2, 3));
	texture_index.push_back(vec3i(0, 2, 3));
	texture_index.push_back(vec3i(0, 3, 1));
	texture_index.push_back(vec3i(0, 3, 2));
	texture_index.push_back(vec3i(0, 1, 3));
	texture_index.push_back(vec3i(0, 3, 2));
	texture_index.push_back(vec3i(0, 1, 3));
	texture_index.push_back(vec3i(0, 1, 3));
	texture_index.push_back(vec3i(0, 3, 2));
...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		vertex_normals.push_back(glm::vec3(0, 1, 0));
		// 这里采用法线来生成颜色，可以自定义自己的颜色生成方式
		vertex_colors.push_back(glm::vec3(0, 1, 0));
	}
	// 中心点
...
void TriMesh::generateCylinder(int num_division, float radius, float height)
{
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		// 添加顶点坐标
		vertex_positions.push_back(glm::vec3(x, y, z));
		vertex_normals.push_back(normalize(glm::vec3(x, 0, z)));
		// 这里颜色和法向量一样
		vertex_colors.push_back(normalize(glm::vec3(x, 0, z)));
	}
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	vertex_positions.push_back(glm::vec3(0, height, 0));
	vertex_normals.push_back(normalize(glm::vec3(0, 1, 0)));
	vertex_colors.push_back(normalize(glm::vec3(0, 0, 0)));
