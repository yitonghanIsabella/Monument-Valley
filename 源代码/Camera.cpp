...
void Camera::updateCamera()
{
	...
	float eyex = x_at + radius * cos(upAngle * M_PI / 180.0) * sin(rotateAngle * M_PI / 180.0);
	float eyey = y_at + radius * sin(upAngle * M_PI / 180.0);
	float eyez = z_at + radius * cos(upAngle * M_PI / 180.0) * cos(rotateAngle * M_PI / 180.0);

	eye = glm::vec4(eyex, eyey, eyez, 1.0);
	at = glm::vec4(x_at, y_at, z_at, 1.0);
}
