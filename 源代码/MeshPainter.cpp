...
void MeshPainter::bindObjectAndDataSky() {
    ...
    skyFaces.push_back("./assets/skybox/right.jpg");
    skyFaces.push_back("./assets/skybox/left.jpg");
    skyFaces.push_back("./assets/skybox/bottom.jpg");
    skyFaces.push_back("./assets/skybox/top.jpg");
    skyFaces.push_back("./assets/skybox/front.jpg");
    skyFaces.push_back("./assets/skybox/back.jpg");   
...
void MeshPainter::drawDynamicMesh(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera, glm::mat4 modelMatrix, bool isShadow) {
    ...
    if (isShadow) {
        glm::mat4 shadowModelMatrix = glm::transpose(light->getShadowProjectionMatrix()) * modelMatrix;
        ...
        glUniform1i(object.shadowLocation, 1);  //isShadow 设置为1
    }
    else {
        glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &modelMatrix[0][0]);
        glUniformMatrix4fv(object.viewLocation, 1, GL_TRUE, &camera->viewMatrix[0][0]);
        glUniformMatrix4fv(object.projectionLocation, 1, GL_TRUE, &camera->projMatrix[0][0]);
        glUniform1i(object.shadowLocation, 0);  //isShadow 设置为0

        // 传递纹理数据 将生成的纹理传给shader
        glUniform1i(glGetUniformLocation(object.program, "material.diffuse"), 0);
...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera, bool isShadow) {
...
void MeshPainter::drawSky(Shader skyboxShader, Camera* camera) {
    // 相机矩阵计算
    camera->updateCamera();
    camera->viewMatrix = camera->getViewMatrix();
    camera->projMatrix = camera->getProjectionMatrix(false);
...
GLuint MeshPainter::loadCubemap(vector<const GLchar*> faces)
{
    GLuint textureID;
    ...
    {
        pixels = stbi_load(faces[i], &width, &height, &channels, 0);
